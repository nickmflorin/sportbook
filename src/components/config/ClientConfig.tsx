...
import css from "./ClientConfig.module.scss";
...
export const ClientConfig = (props: ClientConfigProps) => {
  const [configured, showFade] = useAsyncClientConfiguration();
  ...
  return (
    <MantineProvider>
      <SWRConfig>
        <>
          {configured === true && props.children}
          {(!configured || showFade) && <div className={css["fade-in-screen"]} />}
        </>
      </SWRConfig>
    </MantineProvider>
  );
